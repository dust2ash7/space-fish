(() => {
  ...
  function queueWave(n) {
    const q = [];
    const grunts = 4 + n * 2;
    const spreaders = Math.floor((n + 1) / 2);
    const tanks = n % 3 === 0 ? 1 + Math.floor(n / 9) : 0;
    let t = 0.35;
    for (let i = 0; i < grunts; i++) {
      q.push({ at: t, type: "grunt", x: 50 + (i % 6) * 70 + rand(-12, 12) });
      t += Math.max(0.18, 0.42 - n * 0.015);
    }
    for (let i = 0; i < spreaders; i++) {
      q.push({ at: 1.2 + i * 1.1, type: "spreader", x: 90 + (i % 3) * 150 });
    }
    for (let i = 0; i < tanks; i++) {
      q.push({ at: 2.4 + i * 2, type: "tank", x: W * (0.3 + i * 0.35) });
    }
  ...
  function spawnEnemy(type, x) {
    ...
    if (type === "grunt") {
      Object.assign(base, { hp: 1, r: 13, speed: 95 + G.wave * 6, worth: 100 });
  ...
  function firePlayer() {
    ...
    p.cool = p.rapid > 0 ? 0.09 : 0.2;
    const shots = p.bloom > 0 ? [-0.28, 0, 0.28] : [0];
    shots.forEach((ang) => {
      G.bullets.push({
        x: p.x + Math.sin(ang) * 8,
        y: p.y - 22,
        vx: Math.sin(ang) * 220,
        vy: -520,
        r: 3.4,
  ...
  function setOverlay(name) {
    ...
    if (G.state === "play" && coarse) touchEl.hidden = false;
  ...
  function startGame() {
    ...
    G.state = "play";
    setOverlay(null);
    if (coarse) touchEl.hidden = false;
    hud.hidden = false;
    syncHud(true);
  }
  ...
  function drawKoi(x, y, rot, t, hurt) {
    ...
    const eg = ctx.createRadialGradient(0, 26, 0, 0, 30, 24);
    eg.addColorStop(0, "rgba(124,240,255,0.85)");
    eg.addColorStop(0.45, "rgba(58,160,255,0.28)");
    eg.addColorStop(1, "rgba(58,160,255,0)");
    ctx.fillStyle = eg;
    ctx.beginPath();
    ctx.arc(0, 30, 24, 0, TAU);
    ctx.fill();
    ctx.restore();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(18, 12, 4, 30);
    ctx.quadraticCurveTo(0, 16, -4, 30);
  ...
  function render(dt) {
    ...
    G.bullets.forEach((b) => {
      ...
      ctx.shadowBlur = 8;
      ctx.beginPath();
      ctx.ellipse(b.x, b.y, 2.2, 6, 0, 0, TAU);
      ctx.fill();
      ctx.shadowBlur = 0;
    });
    G.enemies.forEach((e) => {
  ...
  function update(dt) {
    ...
    G.enemies.forEach((e) => {
      ...
      if (e.type === "grunt") {
        ...
        e.cool -= dt;
        if (e.cool <= 0 && e.y > 40 && e.y < H * 0.7) {
          enemyShoot(e);
          e.cool = rand(1.4, 2.2) - G.wave * 0.03;
        }
      } else if (e.type === "spreader") {
        e.y += e.speed * dt;
