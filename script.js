(() => {
  ...
  function queueWave(n) {
    ...
    for (let i = 0; i < grunts; i++) {
      ...
      t += n === 1 ? 0.85 : Math.max(0.22, 0.48 - n * 0.015);
    }
  ...
  function spawnEnemy(type, x) {
    ...
    if (type === "grunt") {
      Object.assign(base, { hp: 1, r: 13, speed: 62 + G.wave * 5, worth: 100 });
  ...
  function render(dt) {
    ...
    G.bullets.forEach((b) => {
      ...
      ctx.shadowBlur = 14;
      ctx.beginPath();
      ctx.ellipse(b.x, b.y, 3.4, 11, 0, 0, TAU);
      ctx.fill();
      ctx.fillStyle = "#7cf0ff";
      ctx.beginPath();
      ctx.ellipse(b.x, b.y + 8, 2.2, 7, 0, 0, TAU);
      ctx.fill();
      ctx.restore();
    });
    G.enemies.forEach((e) => {
  ...
  function update(dt) {
    ...
    if (!G.spawnQ.length && !G.enemies.length && G.waveClock > 9) {
      G.wave += 1;
      queueWave(G.wave);
    }
    steer(dt);
    ...
    G.enemies.forEach((e) => {
      ...
      if (e.type === "grunt") {
        ...
        e.cool -= dt;
        if (e.cool <= 0 && e.y > 80 && e.y < H * 0.62) {
          enemyShoot(e);
          e.cool = (G.wave === 1 ? rand(2.2, 3.0) : rand(1.5, 2.3)) - G.wave * 0.03;
        }
      } else if (e.type === "spreader") {
        e.y += e.speed * dt;
